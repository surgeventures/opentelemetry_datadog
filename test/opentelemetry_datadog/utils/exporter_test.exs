defmodule OpentelemetryDatadog.Utils.ExporterTest do
  use ExUnit.Case, async: true

  @moduletag :unit

  alias OpentelemetryDatadog.Utils.Exporter
  ...
  describe "deep_remove_nils/1" do
    test "removes nil values from maps" do
      ...
      expected = %{a: 1, c: %{d: 2}}
      assert Exporter.deep_remove_nils(input) == expected
    end
    ...
    for {description, input, expected} <- test_cases do
      test "handles edge cases for lists: #{description}" do
        assert Exporter.deep_remove_nils(unquote(Macro.escape(input))) ==
                 unquote(Macro.escape(expected))
      end
    end

    test "handles non-collection types" do
      assert Exporter.deep_remove_nils("string") == "string"
      assert Exporter.deep_remove_nils(42) == 42
      assert Exporter.deep_remove_nils(nil) == nil
    end
  end
  ...
  describe "build_headers/2" do
    ...
    test "includes container ID when provided" do
      headers = Exporter.build_headers(3, "container123")
  ...
  describe "retry_delay/1" do
    ...
    test "delegates to Retry module" do
      exporter_delay1 = Exporter.retry_delay(1)
      retry_delay1 = OpentelemetryDatadog.Core.Retry.retry_delay(1)

      assert exporter_delay1 >= 50 and exporter_delay1 <= 150
      assert retry_delay1 >= 50 and retry_delay1 <= 150

      assert Exporter.retry_delay(0) == 0
